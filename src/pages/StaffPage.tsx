...
import { ArrowLeft, CheckCircle, Plus, ClipboardList, X, Search, UserPlus, Shield, MessageCircle, Send, Download } from 'lucide-react';
...
const emptyForm = { name: '', phone: '', altPhone: '', email: '', age: '', gender: 'Male' as const, shift: 'morning' as Shift, role: '', workType: 'full-time' as WorkType, aadhaarNumber: '', perDaySalary: '' };
...
const StaffPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!form.perDaySalary || parseInt(form.perDaySalary) <= 0) e.perDaySalary = 'Valid salary required';
    setErrors(e);
  ...
  const handleAddStaff = () => {
    ...
    const perDay = parseInt(form.perDaySalary);
    dispatch(addStaffMember({
      id: `st-${Date.now()}`, name: form.name, role: form.role, isManager: false,
      phone: form.phone, altPhone: form.altPhone || undefined, email: form.email,
      ...
      perDaySalary: perDay, salary: perDay * 30,
      attendance: [], leaves: 0, halfDays: 0, salaryPaid: false, tasks: [],
      featureAccess: ['home'],
    }));
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} placeholder="10 digit phone" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                  {errors.phone && <p className="text-xs text-destructive mt-1">{errors.phone}</p>}
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5">Alt Phone</label>
                  <input value={form.altPhone} onChange={e => setForm({...form, altPhone: e.target.value})} placeholder="Alternate number" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1.5">Email *</label>
                <input value={form.email} onChange={e => setForm({...form, email: e.target.value})} placeholder="email@example.com" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                {errors.email && <p className="text-xs text-destructive mt-1">{errors.email}</p>}
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1.5">Age *</label>
                  <input type="number" value={form.age} onChange={e => setForm({...form, age: e.target.value})} className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                ...
                <div>
                  ...
                  <select value={form.gender} onChange={e => setForm({...form, gender: e.target.value as any})} className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm">
                    <option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option>
                  </select>
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1.5">Aadhaar Number</label>
                <input value={form.aadhaarNumber} onChange={e => setForm({...form, aadhaarNumber: e.target.value})} placeholder="XXXX-XXXX-XXXX" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1.5">Role *</label>
                <input value={form.role} onChange={e => setForm({...form, role: e.target.value})} placeholder="e.g. Receptionist, Chef, Security" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                {errors.role && <p className="text-xs text-destructive mt-1">{errors.role}</p>}
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold mb-1.5">Shift *</label>
                  <select value={form.shift} onChange={e => setForm({...form, shift: e.target.value as Shift})} className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm">
                ...
                <div>
                  ...
                  <select value={form.workType} onChange={e => setForm({...form, workType: e.target.value as WorkType})} className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm">
                    <option value="full-time">Full-time</option><option value="part-time">Part-time</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5">Per Day Salary *</label>
                  <input type="number" value={form.perDaySalary} onChange={e => setForm({...form, perDaySalary: e.target.value})} placeholder="₹" className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm" />
                  {errors.perDaySalary && <p className="text-xs text-destructive mt-1">{errors.perDaySalary}</p>}
                </div>
              </div>
            </div>
      ...
      {showBroadcast && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-scale-in" onClick={() => setShowBroadcast(false)}>
          <div className="glass-card rounded-2xl border border-border/50 p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <textarea value={broadcastMsg} onChange={e => setBroadcastMsg(e.target.value)} placeholder="Type your message here..." rows={4}
              className="w-full rounded-xl border border-input bg-background px-3.5 py-2.5 text-sm resize-none" />
    ...
    <div className="max-w-2xl animate-slide-up">
      ...
      <div className="glass-card rounded-2xl border border-border/50 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={downloadProfile} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-muted transition-all">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mb-6">
          ...
          {member.altPhone && <div className="bg-muted/40 rounded-xl p-3"><span className="text-xs text-muted-foreground uppercase tracking-wider">Alt Phone</span><p className="font-semibold mt-0.5">{member.altPhone}</p></div>}
          ...
          {member.aadhaarNumber && <div className="bg-muted/40 rounded-xl p-3"><span className="text-xs text-muted-foreground uppercase tracking-wider">Aadhaar</span><p className="font-semibold mt-0.5">{member.aadhaarNumber}</p></div>}
          <div className="bg-muted/40 rounded-xl p-3"><span className="text-xs text-muted-foreground uppercase tracking-wider">Per Day</span><p className="font-semibold mt-0.5">₹{member.perDaySalary}</p></div>
          <div className="bg-muted/40 rounded-xl p-3"><span className="text-xs text-muted-foreground uppercase tracking-wider">Monthly</span><p className="font-semibold mt-0.5">₹{member.salary.toLocaleString()}</p></div>
